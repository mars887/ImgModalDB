...
## GUI Startup
`gui/main_window.py` now builds a PySide6-based window that starts in fullscreen mode and toggles fullscreen/normal with `F11`. Tabs are laid out as `viewer`, `search`, and `Databases`. The viewer uses reusable `ImageTile` widgets and a scrollable, paginated `ImageGrid` (`gui/widgets/image_grid.py`) with grid density control (3–16 columns) and dual-handle range sliders (`gui/widgets/range_slider.py`) for file size and megapixels. Slider bounds derive from workspace internal stats (min/max file size, min/max megapixels) computed in `WorkspaceManagerV2`, and images are loaded through `WorkspaceManager.list_images` with filters applied plus lazy pagination. Thumbnails load asynchronously via a thread pool to keep the UI responsive. The `Databases` tab uses `DatabasesTab` to manage workspaces and explicit records via a left/right splitter (workspaces on the left, records on the right) and delegates persistence to `WorkspaceManager`.
